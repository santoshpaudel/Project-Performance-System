...
{
    public partial class ListProjectKaryakramPragati : Base
    {
        ...
        {
            int projectId = 0;
            if (e.CommandName == "karyakramPragati")
            {
                projectId = int.Parse(e.CommandArgument.ToString());
                SecureQueryString str = new SecureQueryString();
                str["projectId"] = e.CommandArgument.ToString();
                Response.Redirect(Constants.ConstantAppPath + "/Modules/BarsikKaryekram/KaryekramPragati.aspx" + str.EncryptedString);
            }
        }
