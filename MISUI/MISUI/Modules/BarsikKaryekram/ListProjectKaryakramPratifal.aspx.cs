...
{
    public partial class ListProjectKaryakramPratifal : Base
    {
        ...
        {
            int projectId = 0;
            if (e.CommandName == "pratifalPragati")
            {
                projectId = int.Parse(e.CommandArgument.ToString());
                SecureQueryString str = new SecureQueryString();
                str["projectId"] = e.CommandArgument.ToString();
                Response.Redirect(Constants.ConstantAppPath + "/Modules/OutputTargetManagement/AddProjectOutputProgress.aspx" + str.EncryptedString);
            }
        }
