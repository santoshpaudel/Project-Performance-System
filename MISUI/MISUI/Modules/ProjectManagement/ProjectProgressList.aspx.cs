...
{
    public partial class ProjectProgressList : Base
    {
        ...
        private void PopulateBudgetHead()
        {
            ...
            //dtPopulateBudgetHead = objService1.PopulateAllBudgetHeadsByOfficeId(Session["LanguageSetting"].ToString(), Session["office_id"].ToInt32());
        ...
        {
            int projectId = 0;
            if (e.CommandName == "natijaSuchak")
            {
                projectId = int.Parse(e.CommandArgument.ToString());
                SecureQueryString str = new SecureQueryString();
                str["projectId"] = e.CommandArgument.ToString();
                Response.Redirect(Constants.ConstantAppPath + "/Modules/ProjectManagement/ListProjectTarget.aspx" + str.EncryptedString);
            }
