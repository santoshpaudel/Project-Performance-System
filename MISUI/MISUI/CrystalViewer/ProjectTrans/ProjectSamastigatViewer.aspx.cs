...
{
    public partial class ProjectSamastigatViewer : Base
    {
       // private static DataTable dtMinistries = null;
        ...
        ProjectSamastigatRpt crpt=new ProjectSamastigatRpt();
        ProjectSamastigatRpt crpt1 = new ProjectSamastigatRpt();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (IsPostBack)
            {
                RptViewer.ReportSource = Session["SamastigatReport"];
        ...
        protected void btnFilter_Click(object sender, EventArgs e)
        {
            if (Session["LanguageSetting"].ToString() == "Nepali")
            {
                PopulateProjectSamastigatReport(crpt1);

            }

            else
            {
                PopulateProjectSamastigatReport(crpt);
            }
        }
        ...
        protected void lnkVerify_Click(object sender, EventArgs e)
        {
            ...
            wbs.AuthSoapHdValue = ServiceAuth.ServiceAuthenticationProject();
            int check = 0;
            check = wbs.CheckLowerLevelProjectSamastigatReport(objProjectBO);

            if (check == 1)
            {
                wbs = new ProjectService();
                wbs.AuthSoapHdValue = ServiceAuth.ServiceAuthenticationProject();
                i = wbs.AddProjectSamastigatReportVerification(objProjectBO);
                if (i > 0)
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect",
                            "alert('Report Verified'); window.location='" +
                                Constants.ConstantAppPath + "/CrystalViewer/ProjectTrans/ProjectSamastigatViewer.aspx';", true);
                    //Response.Redirect(Constants.ConstantAppPath + "/CrystalViewer/ProjectTrans/ProjectSamastigatViewer.aspx");
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect",
